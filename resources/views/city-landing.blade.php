...
@push('meta_seo')
<meta name="description" content="Layanan {{ $categoryName }} resmi Kemnaker RI di {{ $city->name }}. Cek jadwal batch, silabus, dan booking kursi.">
<meta property="og:title" content="{{ $categoryName }} di {{ $city->name }} | TrainingKota">
<meta property="og:description" content="Layanan {{ $categoryName }} resmi di {{ $city->name }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div class="lg:col-span-8 space-y-5">
                ...
                <div class="flex flex-wrap items-center gap-4 pt-2">
                    <a href="https://wa.me/{{ config('contact.whatsapp') }}?text={{ urlencode('Halo Admin TrainingKota, saya ingin konsultasi layanan ' . $categoryName . ' untuk area ' . $city->name) }}" target="_blank" class="btn-whatsapp">
                ...
                <tbody class="divide-y divide-[#142338]">
                    <tr class="bg-[#070D18]">
                        ...
                        <td class="py-3.5 px-4 text-right">
                            <a href="https://wa.me/{{ config('contact.whatsapp') }}?text={{ urlencode('Halo Admin TrainingKota, saya booking kursi Ahli K3 Umum untuk batch di ' . $city->name) }}" target="_blank" class="btn-primary text-[11px] py-1.5 px-3">
                                Booking Slot
                            </a>
                        </td>
                    </tr>
                    <tr class="bg-[#0B1526]">
                        ...
                        <td class="py-3.5 px-4 text-right">
                            <a href="https://wa.me/{{ config('contact.whatsapp') }}?text={{ urlencode('Halo Admin TrainingKota, saya booking kursi SMK3 untuk batch di ' . $city->name) }}" target="_blank" class="btn-primary text-[11px] py-1.5 px-3">
                                Booking Slot
                            </a>
                        </td>
                    </tr>
                ...
                <div class="bg-[#0B1526] border border-[#1E324E] p-5 flex flex-col justify-between hover:border-[#0D7A5F] transition-colors">
                    <div>
                        ...
                        <h3 class="font-space font-bold text-sm text-[#F1F5F9] mb-1">
                            <a href="{{ route('city.service.landing', ['category' => $category, 'serviceSlug' => $serv->slug, 'citySlug' => $city->slug]) }}" class="hover:text-[#10B981]">
                                {{ $serv->name }}
                            </a>
                        </h3>
                    ...
                    <div class="pt-4 mt-4 border-t border-[#142338] flex items-center justify-between text-xs">
                        ...
                        <a href="{{ route('city.service.landing', ['category' => $category, 'serviceSlug' => $serv->slug, 'citySlug' => $city->slug]) }}" class="text-[#38BDF8] hover:underline font-space text-[11px]">
                            Detail &rarr;
                        </a>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-4">
                ...
                <a href="https://wa.me/{{ config('contact.whatsapp') }}?text={{ urlencode('Halo Admin TrainingKota, saya punya pertanyaan tentang ' . $categoryName . ' di ' . $city->name) }}" target="_blank" class="btn-whatsapp text-xs">
...
<section class="py-12 bg-[#0B1526]/20 border-b border-[#1E324E]">
    <div class="max-w-7xl mx-auto px-4 lg:px-8">
        <h3 class="font-space font-bold text-sm uppercase text-[#F1F5F9] mb-4">
            Kecamatan di {{ $city->name }}
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($city->kecamatans->unique('name') as $kec)
                <div class="bg-[#0B1526] border border-[#1E324E] p-5 flex flex-col justify-between hover:border-[#0D7A5F] transition-colors">
                    <div class="mb-3">
                        <h4 class="font-space font-bold text-sm text-[#F1F5F9]">
                            {{ $kec->name }}
                        </h4>
                        <p class="text-xs text-[#94A3B8]">Kabupaten/Kota: {{ $city->name }}</p>
                    </div>
                    <a href="{{ route('kecamatan.landing', ['category' => $category, 'kecamatanSlug' => $kec->slug]) }}" class="btn-primary text-xs py-2 px-3 text-center">
                        Lihat layanan
                    </a>
                </div>
            @endforeach
