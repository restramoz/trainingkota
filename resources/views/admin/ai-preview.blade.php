    ...
    <div class="max-w-5xl mx-auto px-4 lg:px-8 py-6 space-y-6">
        ...
        <div class="flex items-center justify-between bg-[#0B1526] border border-[#1E324E] p-4 rounded-lg">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.dashboard', ['tab' => 'articles']) }}"
                   class="px-4 py-2 text-xs font-space uppercase text-[#94A3B8] hover:text-white transition">
                    Cancel
                </a>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            ...
                            <input type="text"
                                   id="preview_title"
                                   value="{{ $data['title'] ?? '' }}"
                                   class="w-full bg-[#080E19] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-sm rounded focus:border-[#0D7A5F] outline-none">
                        </div>
                        <div>
                            <label class="block font-space text-[10px] uppercase text-[#64748B] mb-1">Main Content (HTML)</label>
                            <textarea id="preview_content"
                                      rows="20"
                                      class="w-full bg-[#080E19] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-sm rounded font-mono focus:border-[#0D7A5F] outline-none">{{ $data['content'] ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-[#0B1526] border border-[#1E324E] rounded-lg overflow-hidden">
                    ...
                    <div class="p-5 space-y-3">
                        @if(!empty($data['suggested_faqs']))
                            @foreach($data['suggested_faqs'] as $index => $faq)
                                <div class="p-3 bg-[#070D18] border border-[#1E324E] rounded-lg space-y-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <input type="text"
                                               class="bg-transparent border-none text-[#F1F5F9] text-xs w-full focus:ring-0 p-0"
                                               value="{{ $faq['q'] ?? '' }}">
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] font-bold text-[#64748B]">A</span>
                                        <textarea rows="2"
                                                  class="bg-transparent border-none text-[#94A3B8] text-xs w-full focus:ring-0 p-0">{{ $faq['a'] ?? '' }}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        @else
                ...
                <div class="bg-[#0B1526] border border-[#1E324E] rounded-lg overflow-hidden mt-4">
                    <div class="px-4 py-3 border-b border-[#1E324E] bg-[#0F2038] flex items-center justify-between">
                        ...
                        <button type="button"
                                ...
                                class="text-xs text-[#94A3B8] hover:text-[#F1F5F9]">+ Add Link</button>
                    </div>
                            ...
                            @foreach($data['suggested_internal_links'] as $idx => $link)
                                <div class="flex items-center gap-2 internal-link-row">
                                    <input type="text"
                                           ...
                                           class="bg-transparent border border-[#1E324E] text-[#F1F5F9] text-xs w-1/2 p-1 rounded focus:border-[#0D7A5F]"
                                           value="{{ $link['text'] ?? '' }}">
                                    <input type="text"
                                           placeholder="URL"
                                           class="bg-transparent border border-[#1E324E] text-[#F1F5F9] text-xs w-1/2 p-1 rounded focus:border-[#0D7A5F]"
                                           value="{{ $link['url'] ?? '' }}">
                                    <button type="button"
                                            onclick="removeInternalLink(this)"
                                            class="text-xs text-[#94A3B8] hover:text-[#F1F5F9]">✕</button>
                                </div>
                            @endforeach
                        @else
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   class="w-full bg-[#080E19] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-xs rounded outline-none focus:border-[#0D7A5F]">
                        </div>
                        <div>
                            <label class="block font-space text-[10px] uppercase text-[#64748B] mb-1">Meta Description</label>
                            <textarea id="preview_meta_desc"
                                      rows="3"
                                      class="w-full bg-[#080E19] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-xs rounded outline-none focus:border-[#0D7A5F]">{{ $data['meta_description'] ?? '' }}</textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <textarea id="preview_excerpt"
                                      rows="3"
                                      class="w-full bg-[#080E19] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-xs rounded outline-none focus:border-[#0D7A5F]">{{ $data['excerpt'] ?? '' }}</textarea>
                        </div>
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   class="w-full bg-[#0B1526] border border-[#1E324E] text-[#F1F5F9] px-3 py-2 text-xs rounded outline-none focus:border-[#0D7A5F]">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-[#0F2038] border border-[#1E324E] rounded-lg p-5 space-y-3">
                    ...
                    <button onclick="saveArticle('draft')"
                            class="w-full py-3 bg-[#1E324E] hover:bg-[#2A4263] text-white font-space text-xs font-bold uppercase tracking-widest transition rounded">
                        Save as Draft
                    </button>

                    <button onclick="saveArticle('published')"
                            class="w-full py-3 bg-[#0D7A5F] hover:bg-[#10B981] text-white font-space text-xs font-bold uppercase tracking-widest transition rounded">
                        Publish Now
                    </button>
                </div>
            </div>
...
/**
 * Gather internal links from UI into an array of objects {text, url}
 */
...
async function saveArticle(status) {
    const payload = {
        ...
        status: status,
        // Preserve targeting parameters if they exist in the request (optional)
